// memory.h — 修改版（新增 write_process_memory）
...
#include <linux/highmem.h> // kmap/kunmap函数
...
bool write_process_memory(pid_t pid, uintptr_t addr, const void *buffer, size_t size)
{
	...
	if (!pfn_valid(__phys_to_pfn(pa)) || !kvalid_phys_addr_range(pa, size))
		goto out_mm;
	...
	// 刷新缓存，使写入立刻生效
	for (a = (unsigned long)virt_addr; a < (unsigned long)virt_addr + PAGE_SIZE; a += 64) {
